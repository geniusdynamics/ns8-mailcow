...
until nc php-fpm-mailcow-app 9001 -z; do
  echo "Waiting for PHP on port 9001..."
  sleep 3
done

until nc php-fpm-mailcow-app 9002 -z; do
...
until [[ ! -z ${DOVECOT_V4} ]]; do
  DOVECOT_V4=$(dig a dovecot-mailcow-app +short)
  DOVECOT_V6=$(dig aaaa dovecot-mailcow-app +short)
  [[ ! -z ${DOVECOT_V4} ]] && break;
...
until [[ ! -z ${RSPAMD_V4} ]]; do
  RSPAMD_V4=$(dig a rspamd-mailcow-app +short)
  RSPAMD_V6=$(dig aaaa rspamd-mailcow-app +short)
  [[ ! -z ${RSPAMD_V4} ]] && break;
...
read_servers = "redis-mailcow-app:6379";
write_servers = "${REDIS_SLAVEOF_IP}:${REDIS_SLAVEOF_PORT}";
password = "${REDISPASS}";
timeout = 10;
EOF
  until [[ $(redis-cli -h redis-mailcow-app -a ${REDISPASS} --no-auth-warning PING) == "PONG" ]]; do
    echo "Waiting for Redis @redis-mailcow-app..."
    sleep 2
  done
  ...
  redis-cli -h redis-mailcow-app -a ${REDISPASS} --no-auth-warning SLAVEOF ${REDIS_SLAVEOF_IP} ${REDIS_SLAVEOF_PORT}
else
  cat <<EOF > /etc/rspamd/local.d/redis.conf
servers = "redis-mailcow-app:6379";
password = "${REDISPASS}";
timeout = 10;
EOF
  until [[ $(redis-cli -h redis-mailcow-app -a ${REDISPASS} --no-auth-warning PING) == "PONG" ]]; do
  ...
  redis-cli -h redis-mailcow-app -a ${REDISPASS} --no-auth-warning SLAVEOF NO ONE
fi
...
oletools {
  ...
  servers = "olefy-mailcow-app:10055";
